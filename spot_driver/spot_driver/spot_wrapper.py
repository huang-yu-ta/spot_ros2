    ...
    def hand_pose(self, pose_points):
        try:
            ...
            if not success:
                ...
                # Send the request
                # self._robot_command_client.robot_command(command)
                self._robot_command_client.robot_command(RobotCommandBuilder.build_synchro_command(robot_command))
                self._logger.info('Moving arm to position.')

                time.sleep(6.0)

        except Exception as e:
            return False, "An error occured while trying to move arm: {}".format(e)
